...
beat_thres, db_thres = 0.3, 0.1
file_list = np.sort(glob.glob('./output/*.npz', recursive=True)).tolist() 
...
for file_idx in range(len(file_list)):
    ...
    with open('./output/' + str(name) + '_beat.txt', 'a') as fp:
        fp.writelines(b_out)

    with open('./output/' + str(name) + '_downbeat.txt', 'a') as fp:
        fp.writelines(d_out)
