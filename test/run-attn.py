...
def main():
    ...
    for file_idx in range(len(file_list)):
        ...
        for idx, (x, yb, yd) in enumerate(loader):
            ...
            with open('./tmp/attn/' + str(name) + '/' + str(idx) + '_unit.npz', 'wb') as fp:
                pickle.dump([b, d], fp)

        # output 
        tmp_list = np.sort(glob.glob('./tmp/attn/' + str(name) + '/*.npz', recursive=True)).tolist() 
        b_arr, d_arr = [], []
        ...
        with open('./tmp/attn/' + str(name) + '.npz', 'wb') as f:
